...
class Decoder():
    ...
    def decode(self, args, input, max_length, i, k):
      return decoder_for_gpt3(args, input, max_length, i, k)
...
def setup_data_loader(args):
  ...
  print(f"dataloader_num_workers: {str(dataloader_num_workers)}")
...
def answer_cleansing(args, pred):
  ...
  if args.dataset in ("aqua", "commonsensqa"):
    ...
    pred = pred.replace(",", "")
    pred = list(re.findall(r'-?\d+\.?\d*', pred))
  ...
  if len(pred) == 0:
    pred = ""
  elif (args.method in ("few_shot", "few_shot_cot") and answer_flag
        or args.method not in ("few_shot", "few_shot_cot")
        and args.method in ("zero_shot", "zero_shot_cot")):
...
def create_demo_text(args, cot_flag):
  ...
  if args.dataset not in ("multiarith", "gsm8k"):
    raise ValueError("dataset is not properly defined ...")
